...
The configuration file allows you to edit different aspects of the plugin.

> Adding `${name-of-animation}` will display the given animation instead.

> Adding `%{name-of-placeholder}` will display the given placeholder instead. 

> Adding `${name-of-script:My fancy text}` will display an animation generated by the script `name-of-script` with the input `My fancy text`.

<alert type="warning">

**Issues with special characters?**

If you're having trouble displaying special characters *(unicode characters)*,
then have a look at how to fix your `config.yml` file encoding [here](https://github.com/Puharesource/TitleManager/wiki/Unicode---UTF-8-characters-for-TitleManager).
